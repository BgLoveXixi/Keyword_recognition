...
def prepare_model_settings(label_count, sample_rate, clip_duration_ms,
  ...
  Args:
    ...
    ###model_settings , ,16000, 1000, 40, 20, 40
    #A##audio_processor , , ,10, 10, , 10, 10, dict
  Returns:
    Dictionary containing common settings.
  """
  desired_samples = int(sample_rate * clip_duration_ms / 1000) #16000
  window_size_samples = int(sample_rate * window_size_ms / 1000) #640
  window_stride_samples = int(sample_rate * window_stride_ms / 1000) #320
  length_minus_window = (desired_samples - window_size_samples) #15360
  if length_minus_window < 0:
    spectrogram_length = 0
  else:
    spectrogram_length = 1 + int(length_minus_window / window_stride_samples) #49
  fingerprint_size = dct_coefficient_count * spectrogram_length #40*49
  return {
...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  if model_architecture == 'single_fc':
    ...
    return create_cnn_model(fingerprint_input, model_settings, model_size_info,
                              is_training)
  elif model_architecture == 'trad_cnn_2':
    return create_trad_cnn_2_model(fingerprint_input, model_settings, model_size_info,
                              is_training)
  elif model_architecture == 'mobilenet_1':
    return create_mobilenet_1_model(fingerprint_input, model_settings, model_size_info,
                              is_training)
  elif model_architecture == 'mobilenet_2':
    return create_mobilenet_2_model(fingerprint_input, model_settings, model_size_info,
                              is_training)
...
def create_trad_cnn_2_model(fingerprint_input, model_settings, model_size_info,
                           is_training):
  if is_training:
    dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
  input_frequency_size = model_settings['dct_coefficient_count']  # 40
  ...
  first_pool_height = model_size_info[5]
  first_pool_width = model_size_info[6]

  second_filter_count = model_size_info[7]
  second_filter_height = model_size_info[8]  # time axis
  second_filter_width = model_size_info[9]  # frequency axis
  second_filter_stride_y = model_size_info[10]  # time axis
  second_filter_stride_x = model_size_info[11]  # frequency_axissecond_pool_height = model_size_info[12]
  second_pool_height = model_size_info[12]
  second_pool_width = model_size_info[13]

  first_fc_size = model_size_info[14]
  second_fc_size = model_size_info[15]
  ...
  # first pooling
  first_pool = tf.layers.max_pooling2d(first_dropout,
         [first_pool_height, first_pool_width], [first_pool_height, first_pool_width],
             'valid')
  ...
  second_weights = tf.Variable(
        ...
        stddev=0.01))
  second_bias = tf.Variable(tf.zeros([second_filter_count]))
  second_conv = tf.nn.conv2d(first_pool, second_weights, [
          1, second_filter_stride_y, second_filter_stride_x, 1
           ], 'VALID') + second_bias
  second_conv = tf.layers.batch_normalization(second_conv, training=is_training,
                                            name='bn2')
  ...
  # second pooling
  second_pool = tf.layers.max_pooling2d(second_dropout,
        [second_pool_height, second_pool_width], [second_pool_height, second_pool_width],
        'valid')
  second_pool_shape = second_pool.get_shape()
  second_pool_output_width = second_pool_shape[2]
  second_pool_output_height = second_pool_shape[1]
  second_pool_element_count = int(second_pool_output_width * second_pool_output_height *
                                second_filter_count)
  flattend_second_pool = tf.reshape(second_pool, [-1, second_pool_element_count])

  # first fc
  first_fc_output_channels = first_fc_size
  first_fc_weights = tf.Variable(
        tf.truncated_normal(
        [second_pool_element_count, first_fc_output_channels], stddev=0.01))
  first_fc_bias = tf.Variable(tf.zeros([first_fc_output_channels]))
  first_fc = tf.matmul(flattend_second_pool, first_fc_weights) +   first_fc_bias
  first_fc = tf.layers.batch_normalization(first_fc, training=is_training,
        name='bn3')
  first_fc = tf.nn.relu(first_fc)
  if is_training:
    first_fc_output = tf.nn.dropout(first_fc, dropout_prob)
  else:
    final_fc_output = first_fc

  # second fc
  second_fc_output_channels = second_fc_size
  second_fc_weights = tf.Variable(
        tf.truncated_normal(
        [first_fc_output_channels, second_fc_output_channels], stddev=0.01))
  second_fc_bias = tf.Variable(tf.zeros([second_fc_output_channels]))
  second_fc = tf.matmul(first_fc_output, second_fc_weights) + second_fc_bias
  second_fc = tf.layers.batch_normalization(second_fc, training=is_training,
                                          name='bn4')
  second_fc = tf.nn.relu(second_fc)
  if is_training:
    second_fc_output = tf.nn.dropout(second_fc, dropout_prob)
  else:
    second_fc_output = second_fc
  ...
  final_fc_weights = tf.Variable(
         ...
         [second_fc_output_channels, label_count], stddev=0.01))
  final_fc_bias = tf.Variable(tf.zeros([label_count]))
  final_fc = tf.matmul(second_fc_output, final_fc_weights) + final_fc_bias
  if is_training:
    return final_fc, dropout_prob
  else:
    return final_fc

def create_mobilenet_1_model(fingerprint_input,
          model_settings,  model_size_info, is_training):
  if is_training:
    dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
  width_multiplier = 1
  scope = 'MobileNet1'
  ...
  with tf.variable_scope(scope) as sc:
    ...
    with slim.arg_scope([slim.convolution2d, slim.separable_convolution2d],
      ...
      with slim.arg_scope([slim.batch_norm],
                          ...
                          fused=True):
        net = slim.convolution2d(fingerprint_4d, round(32 * width_multiplier), [3, 3], stride=2, padding='SAME', scope='conv_1')
        ...
        net = slim.avg_pool2d(net, [4, 3], scope='avg_pool_8')
    print('net shape',net.shape)
    end_points = slim.utils.convert_collection_to_dict(end_points_collection)
    net = tf.squeeze(net, [1, 2], name='SpatialSqueeze')
    end_points['squeeze'] = net
    logits = slim.fully_connected(net, num_outputs = 37, activation_fn=None, scope='fc_16')
...
def create_mobilenet_2_model(fingerprint_input,
          model_settings,  model_size_info, is_training):
  if is_training:
    dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
  width_multiplier = 1
  ...
  with tf.variable_scope(scope) as sc:
    ...
    with slim.arg_scope([slim.convolution2d, slim.separable_convolution2d],
      ...
      with slim.arg_scope([slim.batch_norm],
                          ...
                          activation_fn=tf.nn.relu,
                          fused=True):
        net = slim.convolution2d(fingerprint_4d, round(64 * width_multiplier), [3, 3], stride=2, padding='SAME', scope='conv_1')
        net = slim.batch_norm(net, scope='conv_1/batch_norm')
        net = _depthwise_separable_conv(net, 64, width_multiplier, sc='conv_ds_2')
        net = _depthwise_separable_conv(net, 64, width_multiplier, downsample=True, sc='conv_ds_3')
        net = _depthwise_separable_conv(net, 128, width_multiplier, downsample=True, sc='conv_ds_4')
        net = _depthwise_separable_conv(net, 128, width_multiplier, downsample=True, sc='conv_ds_5')
        print('net shape', net.shape)
        net = slim.avg_pool2d(net, [1, 3], stride=1, scope='avg_pool_6')
        net = tf.reshape(net, [-1,1,1,512])
    print('net shape', net.shape)
    end_points = slim.utils.convert_collection_to_dict(end_points_collection)
    net = tf.squeeze(net, [1, 2], name='SpatialSqueeze')
    end_points['squeeze'] = net
    logits = slim.fully_connected(net, num_outputs = 37, activation_fn=None, scope='fc_12')
    #predictions = slim.softmax(logits, scope='Predictions')
...
def create_ds_cnn_15_model(fingerprint_input, model_settings, model_size_info,
                           is_training):
  """Builds a model with depthwise separable convolutional neural network
  Model definition is based on https://arxiv.org/abs/1704.04861 and
  Tensorflow implementation: https://github.com/Zehaos/MobileNet
